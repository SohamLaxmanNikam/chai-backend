...
const generateAccessAndRefereshTokens = async(userId)=>{
    ...
    try{
       ...
       const accessToken =user.gerenareAccessToken()
       const refreshToken=user.gerenareRefreshToken()
...
const loginUser = asyncHandler(async(req,res)=>{
    ...
    // Ensure user object has the method
    if (typeof user.isPasswordCorrect !== "function") {
        throw new ApiError(500, "User model method isPasswordCorrect is missing");
    }
    const isPasswordValid = await user.isPasswordCorrect(password)

    if(!isPasswordValid){
        throw new ApiError(401,"Invalid user credenitlas")
    }
...
const getCurrentUser = asyncHandler(async (req, res) => {
    // Assuming you're using authentication middleware that attaches user info to req.user
    if (!req.user) {
        res.status(401);
...
const updateUserAvatar = asyncHandler(async(req,res)=>{
    ...
    if(!avatarLocalPath){
        new ApiError(400,"Avatar file is misssing")
    }
